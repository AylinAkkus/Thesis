...
We compare our default hyperparameters (LR = $1 \times 10^{-6}$, KL coef = $1 \times 10^{-2}$) with parameters inspired by UI-Venus~\cite{gu2025uivenus} (LR = $4 \times 10^{-7}$, KL coef = $4 \times 10^{-3}$) on the 63k SFT model. Results (Section~\ref{sec:results_ablations}) show robustness to moderate variations in these hyperparameters.
