...
\begin{itemize}
    \item Participated in the search, gathering, and normalization of the existing data sources.
    \item Ran experiments with different filtering methods for the difficulty-based filtering, trained models using SFT, and evaluated the results on the offline benchmarks.
    \item Explored and developed methods to utilize APIs (DOM trees and Accessibility Trees) to supplement training data beyond the web, leveraging richer semantic information to generate instructions.
    \item Participated in ablation experiments on the RL training recipe such as comparing dense vs. sparse rewards, and refusal capability
    \item Developed an agentic harness to evaluate the grounding model end to end on the OS-World benchmark and conducted human evaluation to mitigate the limitations of automated evaluators.
...
\begin{itemize}
    ...
    \item I did not participate in setting up the online OS-World evaluations using containerization and VMs.
\end{itemize}
