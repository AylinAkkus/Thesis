...
# Model difficulty filtering progression data (no pro app data)
...
def create_bar_plot_filtering_only():
    """Create a compact square bar plot with model difficulty filtering stages only."""
    ...
    for model in all_models:
        if 'SFT-7B' in model:
            if '+ Easy+Hard Filter' in model:
                colors.append('#8B0000')  # Dark red for best filtering
    ...
    ax.set_title('Impact of Model Difficulty Filtering\nScreenSpot Pro Performance', 
...
def save_plot():
    ...
    fig = create_bar_plot_filtering_only()
    ...
    # Save as PNG only
    output_path_png = output_dir / 'screenspot_pro_filtering_only.png'
